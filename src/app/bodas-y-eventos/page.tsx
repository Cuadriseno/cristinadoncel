...
import { bodasPage } from "@/content/pages/bodas-y-eventos";
import { createPageMetadata } from "@/lib/metadata";

export const metadata = createPageMetadata(bodasPage.meta.title, bodasPage.meta.description, "/bodas-y-eventos");

export default function BodasYEventosPage() {
  return (
    <>
      <section className="bg-brand-base-2 px-5 py-20 sm:py-28">
        <div className="mx-auto grid max-w-[1200px] items-center gap-12 lg:grid-cols-[1fr_1fr] lg:gap-20">
          <Reveal>
            <h1 className="font-serif font-[200] tracking-[0.03em] text-4xl leading-tight text-brand-secondary sm:text-5xl">
              {bodasPage.hero.headline}
            </h1>
            <p className="mt-7 max-w-xl text-base leading-8 text-brand-text/75 sm:text-lg">
              {bodasPage.hero.subheadline}
            </p>
          </Reveal>
          <Reveal delay={0.1} className="relative aspect-[4/5] min-h-[420px] overflow-hidden sm:min-h-[560px] lg:min-h-0">
            <Image
              src={bodasPage.hero.image.src}
              alt={bodasPage.hero.image.alt}
              fill
              priority
      ...
      <section className="bg-white px-5 pb-20 sm:pb-28">
        <div className="mx-auto max-w-[1200px]">
          <Reveal className="mb-12">
            ...
            <h2 className="font-serif font-[200] tracking-[0.03em] text-3xl text-brand-secondary sm:text-4xl">
              {bodasPage.servicesSection.heading}
            </h2>
          </Reveal>

          <div className="grid gap-8 md:grid-cols-3">
            {bodasPage.services.map((service, index) => (
      ...
      <section className="bg-brand-secondary px-5 py-20 text-white sm:py-24">
        <Reveal className="mx-auto max-w-3xl text-center">
          <h2 className="font-serif font-[200] tracking-[0.03em] text-4xl leading-tight sm:text-5xl">
            {bodasPage.contactCta.heading}
          </h2>
          <p className="mt-6 text-base leading-8 text-white/70">{bodasPage.contactCta.body}</p>
          <Link
            href={bodasPage.contactCta.cta.href}
          ...
          >
            {bodasPage.contactCta.cta.label} <span aria-hidden="true" className="ml-3 text-base">→</span>
          </Link>
        </Reveal>
      </section>
    </>
